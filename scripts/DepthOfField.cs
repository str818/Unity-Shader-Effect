...
public class DepthOfField : MonoBehaviour
{
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        if (_Material)
        {
            ...
            float focalDist = Mathf.Clamp(dist, camera.nearClipPlane, camera.farClipPlane);
            focalDist = focalDist / (camera.farClipPlane - camera.nearClipPlane);
            Debug.Log(focalDist);
            //设置混合材质的纹理
            _BlendMaterial.SetTexture("_BlurTex", rt1);
            _BlendMaterial.SetFloat("_Dist", focalDist);
